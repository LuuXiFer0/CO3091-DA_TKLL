...
#     thresh_binary[(sxbinary==1)|(s_binary==1)] = 1 
    # cv2.imshow("threshold", thresh_binary)
...
def lane_finding(img_orig):
    ...
    try:
        ...
        # 4. Apply a perspective transform to rectify the binary image ("birds-eye view").
        ...
        # Assign values to line_l and line_r
        ...
        # Add the rest of your code outside the function
    except IndexError as e:
        # Handle the IndexError (or any other exception) gracefully
        print(f"An error occurred: {e}")

    # error need to fix
    # fix error when the car go out of the lane 
    return canvas, round(center_diff, 2), 0.0
...
import os
global mtx, dist
calibration_pickle = pickle.load( open( os.path.abspath('src/prius_sdc_pkg/prius_sdc_pkg/Detection/Lanes/calibration_pickle.p'), "rb" ) )
